...
// 用紙は A4 横で固定（この部品の用途＝月次の表の印刷に合わせる）。
//
...
/** 印刷できる範囲（A4 横 297×210mm から余白 8mm を上下左右に引いた大きさ） */
...
const PAGE_CSS = '@page { size: A4 landscape; margin: 8mm; }'
...
function fitToPage(area: HTMLElement, maxPx: number, minPx: number): void {
  const mm = pxPerMm()
  const availW = PRINTABLE_W_MM * mm
  const availH = PRINTABLE_H_MM * mm * FIT_SAFETY
  area.setAttribute('data-cl-measure', '')
  try {
    let size = maxPx
    for (; size > minPx; size -= FONT_STEP_PX) {
      area.style.setProperty('--cl-print-font', `${size}px`)
      if (area.scrollWidth <= availW + 0.5 && area.scrollHeight <= availH) break
    }
...
export const PrintArea = forwardRef<PrintAreaHandle, PrintAreaProps>(function PrintArea(
  { children, maxFontPx = PRINT_MAX_FONT_PX, minFontPx = PRINT_MIN_FONT_PX },
  ref,
) {
  // 印刷する中身の置き場（body の直下）。画面の操作部とは別の木に置くので、印刷の時に中身以外を丸ごと隠せる
  const [host, setHost] = useState<HTMLDivElement | null>(null)
  const limits = useRef({ maxFontPx, minFontPx })
  limits.current = { maxFontPx, minFontPx: Math.max(PRINT_MIN_FONT_PX, minFontPx) }
  ...
  const prepare = useCallback(() => {
    ...
    injectPageStyle()
    fitToPage(host, limits.current.maxFontPx, limits.current.minFontPx)
  }, [host])
